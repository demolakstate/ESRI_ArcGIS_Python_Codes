...
where_clause = "ACRES >= 10 AND ACRES < 40"
#try:
...
arcpy.FeatureClassToFeatureClass_conversion(featureLayer, os.getcwd(), "Parcels_ORION_map", "#", mapS)
...
# Feature to point
arcpy.FeatureToPoint_management("Parcels_ORION_map", os.path.join(os.getcwd(), "Parcels_ORION_mappoint"), 'INSIDE')
...
fc = os.path.join(os.getcwd(), "Parcels_ORION_map")
#field_TRS = ['TRS']
field_TNP = ['TNP_RNG']

values = [row[0] for row in arcpy.da.SearchCursor(fc, field_TNP)]
uniqueValues = set(values)

arcpy.AddMessage("TRS {0}".format(uniqueValues))

in_features = os.path.join(os.getcwd(), "Parcels_ORION_map")
...
if not os.path.exists(wall_map):
    ...
    arcpy.CreateFeatureDataset_management(out_path, wall_map, spatial_reference) # projection file
...
#os.chdir(plat_book)
change_path = os.path.join(os.getcwd(), "Wall_Map")
